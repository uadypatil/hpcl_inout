...
$sql = "SELECT * from `feg` where `token_no`=$token";
...
if (isset($_POST['update'])) {
     ...
     if(mysqli_num_rows($resa) > 0){
         ...
         $sqlo = "SELECT * FROM feg where aadhar_no= '$aadhar_no'";
         ...
         if ( mysqli_num_rows($reso) > 0) {
          ...
          if( $token_number == $token_no && $aadhar_number == $aadhar_no){
               ...
               $sql = "UPDATE feg SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address' WHERE token_no='$token_no'";
               $res = mysqli_query($connection, $sql);
...
if (isset($_POST['drop'])) {
     ...
     $sql = "UPDATE `feg` SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `id`='$token'";
     $res = mysqli_query($connection, $sql);
...
function isQrGenerated($token_no)
{
    ...
    $sql = "SELECT `qr_code` FROM `feg` WHERE `token_no` = $token_no";
    global $connection;
    $res = mysqli_query($connection, $sql);
    if ($res) {
...
{
    global $connection;
    $sql = "SELECT `id`, `token_no`, `working_as` FROM `feg` WHERE `token_no` = '$token_no'";
    $res = mysqli_query($connection, $sql);
    $working_as = "";
 ...
 if ($submodule == 'feg' || $submodule == 'feg') { 
  $submodule = "FEG";
}
...
//     pw/hpnsk/1
    ...
    $sqlUpdate = "UPDATE `feg` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
    header("Location: fegicard.php?token=$token_no");
}
...
<body>
                              ...
                              <form method="post" id="myform" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="text" name="aadhar_no" id="aadhar_no" class="form-control" oninput="this.value=this.value.replace(/[^0-9]/g,'')" size="12" minlength="12" maxlength="12"
                                                  value="<?php echo $row['aadhar_no']; ?>" placeholder="Enter Aadhar Number">
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <textarea name="address" id="address" class="form-control" oninput="this.value=this.value.replace(/[^a-z\sA-Z0-9,.-]/g,'');ws(this.id);" placeholder="Enter Address"
                                        ...
                                        <div class="d-grid gap-2 d-sm-block">
                                             <div>
                                                  ...
                                                  <button class="btn btn-secondary" name="idcard-btn" id="idcard-btn"><i
                                                       ...
                                                       Card</button>
                                                       <a href="feg.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                                       Back</a>
     ...
     <script>
          document.getElementById('page-title').innerHTML = "HPCL INOUT | FEG";
          document.getElementById('navbar-title').innerHTML = "Edit Feg Form";
     </script>
