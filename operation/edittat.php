...
$sql = "SELECT * from `tat` where `token_no`=$token";
...
if(isset($_GET['token'])){
    ...
    echo "Invalid page! <a id='backbtn' href='../login.php'>go to login page</a>";die;
     ...
     if(mysqli_num_rows($resa) > 0){
         ...
         $sqlo = "SELECT * FROM tat where aadhar_no= '$aadhar_no'";
         ...
         if ( mysqli_num_rows($reso) > 0) {
          ...
          if( $token_number == $token && $aadhar_number == $aadhar_no){
               ...
               $sql = "UPDATE tat SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address' WHERE token_no='$token'";
               $res = mysqli_query($connection, $sql);
...
if (isset($_POST['drop'])) {
     ...
     $sql = "UPDATE `tat` SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `id`='$token'";
...
function isQrGenerated($token_no)
{
     ...
     $sql = "SELECT `qr_code` FROM `tat` WHERE `token_no` = $token_no";
     global $connection;
     $res = mysqli_query($connection, $sql);
     if ($res) {
...
{
     global $connection;
     $sql = "SELECT `id`, `token_no`, `working_as` FROM `tat` WHERE `token_no` = '$token_no'";
     $res = mysqli_query($connection, $sql);
     $working_as = "";
...
//     pw/hpnsk/1
     ...
     $sqlUpdate = "UPDATE `tat` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
     header("Location: taticard.php?token=$token_no");
}
...
<body>
                         ...
                         <form method="post" id="myform" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                              ...
                              <div class="row">
                                   <div class="col-md-12 mb-4">
                                        ...
                                        <textarea name="address" class="form-control" id="address" oninput="this.value=this.value.replace(/[^a-z\sA-Z0-9,.-]/g,'');ws(this.id);" rows="3" placeholder="Enter Address"
                                   ...
                                   <div class="d-grid gap-2 d-sm-block">
                                        <div>
                                             ...
                                             <button class="btn btn-secondary" name="idcard-btn"><i
                                                  ...
                                                  Card</button>
                                                  <a href="tat.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                                  Back</a>
     ...
     <script>
          ...
          document.getElementById('navbar-title').innerHTML = "Edit TAT Form";
     </script>
