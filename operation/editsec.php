...
$sql = "SELECT * from `sec` where `token_no`=$token";
...
if (isset($_POST['update'])) {
     ...
     if(mysqli_num_rows($resa) > 0){
         ...
         $sqlo = "SELECT * FROM sec where aadhar_no= '$aadhar_no'";
...
if (isset($_POST['drop'])) {
     ...
     $sql = "UPDATE `sec` SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL ,`firm_name`=NULL,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `id`='$token'";
...
function isQrGenerated($token_no)
{
     ...
     $sql = "SELECT `qr_code` FROM `sec` WHERE `token_no` = $token_no";
     global $connection;
     $res = mysqli_query($connection, $sql);
     if ($res) {
...
{
     global $connection;
     $sql = "SELECT `id`, `token_no`, `working_as` FROM `sec` WHERE `token_no` = '$token_no'";
     $res = mysqli_query($connection, $sql);
     $working_as = "";
     ...
     if ($submodule == 'Sec' || $submodule == 'sec') {
          $submodule = "SEC";
     }
...
//     pw/hpnsk/1
     ...
     $sqlUpdate = "UPDATE `sec` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
     header("Location: secicard.php?token=$token_no");
}
...
<body>
                              ...
                              <form method="post" id="myform" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="type" name="address" class="form-control" id="address" oninput="this.value=this.value.replace(/[^a-z\sA-Z0-9,.-]/g,'');ws(this.id);" placeholder="Enter Address"
                                                  required rows="3" required value="<?php echo $row['address']; ?>">
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="text" name="firm_name" id="firm_name"
                                                  oninput="this.value=this.value.replace(/[^a-z\sA-Z]/g,'');ws(this.id);"
                                                  class="form-control" value="<?php echo $row['firm_name']; ?>"
     ...
     <script>
          ...
          document.getElementById('navbar-title').innerHTML = "Edit SEC Form";
     </script>
