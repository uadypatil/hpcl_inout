...
$sql = "SELECT * from officer where token_no=$token";
...
if (isset($_POST['update'])) {
    ...
    $sqlo = "SELECT * FROM officer where aadhar_no= '$aadhar_no'";
    ...
    if ( mysqli_num_rows($reso) > 0) {
     ...
     if( $token_number == $token_no && $aadhar_number == $aadhar_no){
          ...
          $sql = "UPDATE officer SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address' WHERE token_no='$token_no'";
          $res = mysqli_query($connection, $sql);
...
if (isset($_POST['drop'])) {
     ...
     $sql = "UPDATE `officer` SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `token_no`='$token'";
     $res = mysqli_query($connection, $sql);
...
{
     // header("Location: officericard.php?token=$token_no");
     header("Location: officericard.php?token=$token_no");
...
function isQrGenerated($token_no)
{
     ...
     $sql = "SELECT `qr_code` FROM `officer` WHERE `token_no` = $token_no";
     global $connection;
     $res = mysqli_query($connection, $sql);
     if ($res) {
...
{
     global $connection;
     $sql = "SELECT `id`, `token_no`, `working_as` FROM `officer` WHERE `token_no` = '$token_no'";
     $res = mysqli_query($connection, $sql);
     $working_as = "";
     ...
     $sqlUpdate = "UPDATE `officer` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
                              ...
                              <form method="post" id="myform" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>"
                                   ...
                                   <div class="row mt-2">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="text" name="token_no" name="token_no"
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <textarea name="address" id="address" class="form-control" placeholder="Enter Address"
                                                  rows="3" oninput="this.value=this.value.replace(/[^a-z\sA-Z0-9,.-]/g,'');ws(this.id);"><?php echo $row['address']; ?></textarea>
                                   ...
                                   <div class="d-grid gap-2 d-sm-block">
                                        <div>
                                             ...
                                             <button class="btn btn-secondary" name="idcard-btn"><i
                                                  ...
                                                  Card</button>
                                                  <a href="officer.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                                  Back</a>
...
function ws(name) { 
        ...
        name.value = name.value.replace(/^\s+/, '');
          ...
          document.getElementById('page-title').innerHTML = "HPCL INOUT | Officer";
          document.getElementById('navbar-title').innerHTML = "Edit Officer Form <i class='fa-solid fa-users'></i>";
