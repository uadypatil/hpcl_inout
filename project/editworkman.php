...
$sql = "SELECT * from workman where token_no=$token";
...
if(mysqli_num_rows($resa) > 0){
  ...
  $sqlo = "SELECT * FROM workman where aadhar_no= '$aadhar_no'";
  ...
  if ( mysqli_num_rows($reso) > 0) {
   ...
   if( $token_number == $token && $aadhar_number == $aadhar_no){
        ...
        $sql = "UPDATE workman SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address',firm_name='$firm_name' WHERE token_no = '$token'";
        $res = mysqli_query($connection, $sql);
        ...
        if ($res && $res1) {
            ...
            echo '<script>
                window.location.href = "workman.php";
        </script>';
        } else {
...
if (isset($_POST['drop'])) {
     ...
     // $sql2 = "UPDATE `workman` SET aadhar_no=NULL,full_name=NULL,mobile_no=NULL,address=NULL,firm_name=NULL,truck_no=NULL WHERE id='$token'";

 $sql2 = "UPDATE `workman` SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL ,`firm_name`=NULL ,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `token_no`='$token'";
...
function isQrGenerated($token_no)
{
    ...
    $sql = "SELECT `qr_code` FROM `workman` WHERE `token_no` = $token_no";
    global $connection;
    $res = mysqli_query($connection, $sql);
    if ($res) {
...
{
    global $connection;
    $sql = "SELECT `id`, `token_no`, `working_as` FROM `workman` WHERE `token_no` = '$token_no'";
    $res = mysqli_query($connection, $sql);
    $working_as = "";
...
//  }
 ...
 $module = "";
    ...
    if ($submodule == 'workman' || $submodule == 'WORKMAN' || $submodule == 'Workman') {
        $submodule = "PW";
    }
...
//     pw/hpnsk/1
    ...
    $sqlUpdate = "UPDATE `workman` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
    header("Location: workmanicard.php?token=$token_no");
}
...
<body>
   ...
   <div class="card border-0 shadow mt-4">
    ...
    <form method="post" action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" enctype="multipart/form-data" id="myform">
            <input type="hidden" name="id" value="<?php echo $row['id'];?>">
                ...
                <div class="row">
                    ...
                    <label for="address">Address :</label>
                                             <textarea name="address" id="address" class="form-control" oninput="this.value=this.value.replace(/[^a-z\sA-Z0-9,.-]/g,'');ws(this.id);"  
                                                  rows="3" placeholder="Enter Address"><?php echo $row['address']; ?></textarea>
                ...
                <div class="row">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="firm_name" id="firm_name" oninput="this.value=this.value.replace(/[^a-z\sA-Z]/g,'');ws(this.id);checkfirm();" class="form-control" required value="<?php echo $row['firm_name']?>" placeholder="Enter Firm Name">
                ...
                <div>
                <button class="btn btn-primary" id="update" name="update"><i class="fa-solid fa-user-pen"></i></i>  Update</button> 
                ...
                <button type="submit" name="idcard-btn" id="idcard-btn" class="btn btn-secondary">
                                    ...
                                    <a href="workman.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                    Back</a>
     ...
     <script>
          document.getElementById('page-title').innerHTML = "HPCL INOUT | Workman";
          document.getElementById("navbar-title").innerHTML = "Edit Workman <i class='fa-solid fa-users'></i>";
     </script>
