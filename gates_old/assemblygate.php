          ...
          if ($parts['1'] == "HPNSK") {
               ...
               switch ($sub_dept) {
                    ...
                    case 'FEG':
                         ...
                         break;
                    case 'SEC':
                         setValue("feg", "operation", "SEC", $id, $qr);
                         break;
          ...
          if (isQrExist($id, $sub_dept)) {
               ...
               if($res){
                    ...
                    if($row){
                         ...
                         $sql2 = "UPDATE `assemblygate` SET `outtime`='" . date('H:i:s') . "', `status`= 0 WHERE  `token_no` = '$id' AND `date` = CURDATE() AND `status` = 1 AND TIMESTAMPDIFF(SECOND, `intime`, NOW()) >= 3";
                         $res2 = mysqli_query($connection, $sql2);
                         if ($res2 && mysqli_affected_rows($connection) > 0) {
                         $success_msg = getFullName($table, $id) . " Scan Out To Assembly gate";
                          // echo "person exited";
                         }else{
                              $error_msg = getFullName($table, $id) . " Can't Scan Out To Assemnly gate Now";
                         }    
                    } else {
               ...
               if(qrInMaingate($qr) || qrInLicensegate($qr)){
                    $sql = "SELECT * FROM $table WHERE `token_no` = '$id'  AND `restricted` = '0'";
                    $res = mysqli_query($connection, $sql);
                    ...
                    if($res){
                         ...
                         if($row){
                              ...
                              // Update query
                              $sqlmain = "UPDATE `maingate` SET outtime = '$defaultOutTime', `status` = 0 WHERE `qr_code`='$qr' AND `status` = 1";
                              $res1 = mysqli_query($connection, $sqlmain);
                              $sqllic = "UPDATE `licensegate` SET outtime = '$defaultOutTime', `status` = 0 WHERE `qr_code`='$qr' AND `status` = 1";
                              $res2 = mysqli_query($connection, $sqllic);
                              $sql2 = "INSERT INTO `assemblygate`(`qr_code`, `intime`, `date`, `section`, `token_no`, `department`, `adhar`, `name`, `mobile`, `address`) VALUES (
                              '" . $row['qr_code'] . "', '" . date('H:i:s') . "', '" . date("Y-m-d") . "', '$sub_dept', '$id', '" . $dept . "', '" . $row['aadhar_no'] . "', '" . $row['full_name'] . "', '" . $row['mobile_no'] . "', '" . $row['address'] . "')";
                              $res3 = mysqli_query($connection, $sql2);
                               // echo "<br>".$sql2;
                              if ($res1 && $res2 && $res3) {
                              $success_msg = getFullName($table, $id) . " Scan In To Assembly gate";
                              // echo "Person entered.";
                              }
                    ...
                    $error_msg = getFullName($table, $id) . " is not in plant";
               }
...
// checks is person entered in maingate
function qrInMaingate($qr){
     global $connection;

     $sql = "SELECT * FROM `maingate` WHERE `qr_code`='$qr' AND `date` = CURDATE() AND `status`=1";
     $res = mysqli_query($connection, $sql);
...
// checks is person entered in licensegate
function qrInLicensegate($qr){
     global $connection;

     $sql = "SELECT * FROM `licensegate` WHERE `qr_code`='$qr' AND `date` = CURDATE() AND `status`=1";
     $res = mysqli_query($connection, $sql);
...
// checks is person is in assembly gate or not
function isQrExist($token, $sub_dept)
{
     global $connection;

     $sql = "SELECT * FROM `assemblygate` WHERE `token_no` = '$token' AND `section`='$sub_dept' AND `date` = CURDATE() AND `status`='1' ";
     $res = mysqli_query($connection, $sql);
...
<!-- filename -->
<?php
$filename = "assemblygate.php";
?>

<body>

     <?php // include "maingatecontent.php"; ?>
     <?php include "assemblygatecontent.php"; ?>
