...
function setValue($table, $dept, $sub_dept, $id, $qr)
{
     ...
     if(qrintable($table, $qr)){
          ...
          if (isQrExist($id, $sub_dept)) {
               if (isInLicenseGate($id, $sub_dept)) {
                    $error_msg = "First exit License area";
                    ...
                    if($res){
                         ...
                         if($row){
                              ...
                              // UPDATE `maingate`
                              //      SET `outtime` = NOW(), `status` = 0
                              //      WHERE `token_no` = '1'
                              //      AND `qr_code` = 'OFC/HPNSK/1'
                              //      AND `date` = CURDATE()
                              //      AND `status` = 1
                              //      AND TIMESTAMPDIFF(SECOND, `intime`, NOW()) >= 120;

                              $sql2 = "UPDATE `maingate` SET `outtime`='" . date('H:i:s') . "', `status`= 0 WHERE `token_no` = '$id' AND `qr_code` = '$qr' AND `date` = CURDATE() AND `status` = 1 AND TIMESTAMPDIFF(SECOND, `intime`, NOW()) >= 3";
                              // echo $sql2;die;
                              $res2 = mysqli_query($connection, $sql2);
                              if ($res2 && mysqli_affected_rows($connection) > 0) {
                                   $success_msg = getFullName($table, $id) . " Scan Out To Main gate";
                                   // echo "person exited";
                              }else{
                                   $error_msg = getFullName($table, $id) . " Can't Scan Out To Main gate Now";
                              }
               ...
               if (isInAssemblyGate($id, $sub_dept)) {
                    $error_msg = "First exit Assembly area"; // echo "person is in license gate"; 
               } else {
                         ...
                         if($row){
                              $sql2 = "INSERT INTO `maingate`(`qr_code`, `intime`, `date`, `section`, `token_no`, `department`, `adhar`, `name`, `mobile`, `address`) VALUES (
                                   '" . $row['qr_code'] . "', '" . date('H:i:s') . "', '" . date("Y-m-d") . "', '$sub_dept', '$id', '" . $dept . "', '" . $row['aadhar_no'] . "', '" . $row['full_name'] . "', '" . $row['mobile_no'] . "', '" . $row['address'] . "')";
                                       ...
                                       // echo "<br>".$sql2;
                                       if ($res2) {
                                            $success_msg = getFullName($table, $id) . " Scan In To Main gate";
                                            // echo "Person entered.";
                                       }
                         } else {
...
// checks is person is in maingate or not
...
// checks person is in license gate or not
function isInLicenseGate($token, $sub_dept)
{
     global $connection;
...
// checks person is in license gate or not
function isInAssemblyGate($token, $sub_dept)
{
     global $connection;
...
<body>
     ...
     <script>
          ...
          function checkqr() {
               ...
               window.location.href = 'addmaingate.php?qc=' + qr_code;
          }
     </script>
     <script>
          document.getElementById('page-title').innerHTML = "Main Gate";
     </script>
