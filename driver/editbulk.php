...
$sql = "SELECT * from bulk where token_no=$token";
...
if (isset($_POST['update'])) {
     ...
     if(mysqli_num_rows($resa) > 0){
         ...
         $sqlo = "SELECT * FROM bulk where aadhar_no= '$aadhar_no'";
         ...
         if ( mysqli_num_rows($reso) > 0) {
          ...
          if( $token_number == $token_no && $aadhar_number == $aadhar_no){
               ...
               $sql = "UPDATE bulk SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address',firm_name='$firm_name',truck_no='$truck_no' WHERE token_no = '$token_no'";
               $res = mysqli_query($connection, $sql);
...
if (isset($_POST['drop'])) {
     ...
     $sql2 = "UPDATE bulk SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL,`firm_name`=NULL,`truck_no`=NULL ,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `token_no`='$token'";
     ...
     if ($res2 && $reszi) {
          // If $res2 is true, redirect to bulk.php
...
function isQrGenerated($token_no)
{
     ...
     $sql = "SELECT `qr_code` FROM `bulk` WHERE `token_no` = $token_no";
     global $connection;
     $res = mysqli_query($connection, $sql);
     if ($res) {
...
{
     global $connection;
     $sql = "SELECT `id`, `token_no`, `working_as` FROM `bulk` WHERE `token_no` = '$token_no'";
     $res = mysqli_query($connection, $sql);
     $working_as = "";
     ...
     $sqlUpdate = "UPDATE `bulk` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
     header("Location: bulkidcard.php?token=$token_no");
}
...
<body>
                                   ...
                                   <div class="d-grid gap-2 d-sm-block">
                                        <div>
                                             ...
                                             <button class="btn btn-secondary" name="idcard-btn" id="idcard-btn"><i
                                                  ...
                                                  Card</button>
                                                  <a href="bulk.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                                  Back</a>
     ...
     <script>
          document.getElementById('page-title').innerHTML = "HPCL INOUT | Bulk Form";
          document.getElementById('navbar-title').innerHTML = "Edit Bulk Form <i class='fa-solid fa-users'></i>";
     </script>
