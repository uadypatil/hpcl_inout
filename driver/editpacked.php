...
$sql = "SELECT * from packed where token_no=$token";
...
if (isset($_POST['update'])) {
     ...
     if(mysqli_num_rows($resa) > 0){
         ...
         $sqlo = "SELECT * FROM packed where aadhar_no= '$aadhar_no'";
         ...
         if ( mysqli_num_rows($reso) > 0) {
          ...
          if( $token_number == $token_no && $aadhar_number == $aadhar_no){
               ...
               $sql = "UPDATE packed SET aadhar_no='$aadhar_no',full_name='$full_name',mobile_no='$mobile_no',address='$address',firm_name='$firm_name',truck_no='$truck_no' WHERE token_no = '$token_no'";
               $res = mysqli_query($connection, $sql);
            ...
            echo '<script>
                window.location.href = "packed.php";
        </script>';
               } else {
...
if (isset($_POST['drop'])) {
     ...
     $sql2 = "UPDATE packed SET `aadhar_no`=NULL,`full_name`=NULL,`mobile_no`=NULL,`address`=NULL,`firm_name`=NULL,`truck_no`=NULL ,`qr_code`=NULL, `qr_path`=NULL,`restricted` = '0' WHERE `token_no`='$token'";
     ...
     if ($res2 && $reszi) {
          // If $res2 is true, redirect to packed.php
          $_SESSION['flash_message']="Data Deleted";
            echo '<script>
                window.location.href = "packed.php";
        </script>';
          // header("Location: packed.php");
...
function isQrGenerated($token_no)
{
     ...
     $sql = "SELECT `qr_code` FROM `packed` WHERE `token_no` = $token_no";
     global $connection;
     $res = mysqli_query($connection, $sql);
     if ($res) {
...
{
     global $connection;
     $sql = "SELECT `id`, `token_no`, `working_as` FROM `packed` WHERE `token_no` = '$token_no'";
     $res = mysqli_query($connection, $sql);
     $working_as = "";
     ...
     $sqlUpdate = "UPDATE `packed` SET `qr_code`='$finalQr' ,`qr_path`='$filename'  WHERE `token_no` = '$token_no'";
...
{
     header("Location: packageidcard.php?token=$token_no");
}
...
<body>
                                   ...
                                   <div class="row">
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="text" name="aadhar_no" id="aadhar_no" class="form-control"
                                                  oninput="this.value=this.value.replace(/[^0-9]/g,'');checkadhar();"
                                   ...
                                   <div class="row">
                                        ...
                                        <div class="col-md-12 mb-4">
                                             ...
                                             <input type="text" name="firm_name" id="firm_name" class="form-control"
                                                  ...
                                                  oninput="this.value=this.value.replace(/[^a-z\sA-Z]/g,'');ws(this.id);"
                                                  value="<?php echo $row['firm_name']; ?>" required>
                                             <span id="firmerror" style="color: red;"></span>

                                        </div>
                                        <div class="col-md-12 mb-4">
                                             <label for="truck_no">Truck No :</label>
                                             <input type="text" name="truck_no" placeholder="Enter Truck No."
                                                  id="truck_no"
                                                  oninput="this.value=this.value.replace(/[^a-zA-Z0-9\s]/g, '');ws(this.id);"
                                                  class="form-control" value="<?php echo $row['truck_no']; ?>" required>
                                   ...
                                   <div class="d-grid gap-2 d-sm-block">
                                        <div>
                                             ...
                                             <button class="btn btn-secondary" name="idcard-btn" id="idcard-btn"><i
                                                  ...
                                                  Card</button>
                                                  <a href="packed.php"  class="btn btn-danger" ><i class="fa-solid fa-arrow-left"></i>
                                                  Back</a>
     ...
     <script>
          document.getElementById('page-title').innerHTML = "HPCL INOUT | Packed Form";
          document.getElementById('navbar-title').innerHTML = "Edit Packed Form <i class='fa-solid fa-users'></i>";
     </script>
     ...
     <script>
          ...
          truckField.addEventListener("input", validateTruck);
          ...
          function validateTruck() {
               var truck = truckField.value;
               var errorElement = document.getElementById("truckerror");

               if (truck.trim() == "") {
                    errorElement.textContent = "Fill Out This Field";
                    truckField.classList.add("invalid");
                    return false;
               }
               else {
                    errorElement.textContent = "";
                    truckField.classList.remove("invalid");
                    return true;
          ...
          form.addEventListener('submit', (event) => {
               ...
               if (!validateContact() || !validateAadharNo() || !validateName() || !validateAddr() || !validateFirm() || !validateTruck()) {
                    event.preventDefault();
               }
